...
```bash
uv run pytest tests/dashboard -q > /tmp/pytest.log 2>&1; echo "pytest exit=$?"; tail -n 15 /tmp/pytest.log
...
npm run lint > /tmp/lint.log 2>&1; echo "lint exit=$?"; tail -n 20 /tmp/lint.log
...
- If non-zero: `grep -nE "FAILED|Error|error:" /tmp/<log>` (or open just the failing test
  ...
  `uv run pytest tests/dashboard/test_<x>.py -q`.
