...
| **P4** | Transactions (dashboard roster-diff tier) | analytics + api + frontend | — | F-37 (tier 1) | ◐ (build done; VERIFY gate green, real-DB sign-off **BLOCKED by F-53**) | `docs/plans/fix-P4-transactions.md` |
...
| **UP** | Upstream / Phase-1 program & research (NOT dashboard PRs) | data (pipeline) + research | runs alongside | F-27 (data half ✅ landed → F-51), F-25, F-37 (tier 2), F-06, F-49, ~~F-50~~ ✅ regen, F-52 (likely ✅ later regen), **F-53 (blocks P4)** | ☐ | per-program handoffs in `docs/handoffs/` |
