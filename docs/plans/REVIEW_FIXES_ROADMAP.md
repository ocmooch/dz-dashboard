...
| **P2** | Data honesty & affordance precision (+ gap-validation harness) | gap-affordance + tests | soft: P1 (week semantics) | F-16, F-35, F-26, F-33, F-48, F-43; post-regen redo for F-51 | ☑ redo verified (PR pending) | `docs/plans/fix-P2-honesty.md`; redo: `docs/plans/fix-P2-post-regen-redo.md` |
| **P3** | Search: scope, teams, hardening | data/analytics + api + tests | — | F-44, F-45, F-47 | ☑ PR #32 (real-DB click-through done post-regen; F-50 resolved) | `docs/plans/fix-P3-search.md` |
| **P4** | Transactions (dashboard roster-diff tier) | analytics + api + frontend | — | F-37 (tier 1) | ◐ (build done; gate green; **F-53 fixed → real-DB verified 2026-06-06**; only PR to `dev` left) | `docs/plans/fix-P4-transactions.md` |
...
| **UP** | Upstream / Phase-1 program & research (NOT dashboard PRs) | data (pipeline) + research | runs alongside | F-27 (data half ✅ landed → F-51), F-25, F-37 (tier 2), F-06, F-49, ~~F-50~~ ✅ regen, F-52 (likely ✅ later regen), ~~F-53~~ ✅ regen (wk1 fixed) | ☐ | per-program handoffs in `docs/handoffs/` |
